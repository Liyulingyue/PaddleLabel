...
import { Spin, message, Button, Modal, Input, ColorPicker, List, Popconfirm } from 'antd';
...
import PPSetButton from '@/components/PPSetButton';
...
export default function SemanticSegmentation() {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    else if (e.key === 'm' || (e.ctrlKey && e.key === 's')) { e.preventDefault(); saveAnnotations(); }
  ...
  const drawToolParam = useMemo(() => ({
    ...
    isLabel: '',
    brushSize,
  }), [currentData?.dataId, selectedLabel, scale, currentTool, annotations, selectedAnnotation, frontendId, onAnnotationAdd, onAnnotationModify, brushSize]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <PageHeader projectId={projectId} categoryLabel={t('global.semanticSegmentation')} />
      ...
      <div className="toolbarLeft">
        <PPToolBarButton
          ...
          onClick={() => {
            if (!selectedLabel) { message.error(t('pages.toolBar.chooseCategoryFirst')); return; }
        ...
        <PPSetButton
          ...
          onChange={(size) => { if (size >= 1) setBrushSize(size); }}
        >
          {t('pages.toolBar.brush')}
        </PPSetButton>
        <PPSetButton
          imgSrc={`${BTN}rubber.png`}
          size={brushSize}
          active={currentTool === 'rubber'}
          onClick={() => setCurrentTool('rubber')}
          onChange={(size) => setBrushSize(size)}
        >
          {t('pages.toolBar.rubber')}
        </PPSetButton>
        <PPToolBarButton
        ...
        <PPSetButton
          imgSrc={`${BTN}alpha.png`}
          size={transparency}
          maxSize={100}
          minSize={0}
          onChange={(size) => setTransparency(size)}
        >
          {t('pages.toolBar.transparency')}
        </PPSetButton>
      </div>
      ...
      <div className="rightSideBar">
        ...
        <List
          ...
          bordered
          dataSource={annotations.filter(a => a.type !== 'rubber')}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                ...
                <Button
                  ...
                  type="text"
                  danger
                  onClick={(e) => { e.stopPropagation(); onAnnotationDelete(item); }}
                >
                  ×
                </Button>
              </div>
